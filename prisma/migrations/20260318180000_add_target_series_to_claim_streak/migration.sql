-- Add target series support to claim_login_streak_reward
-- Allows users to specify target series when claiming streak rewards

DO $$
BEGIN
  -- Update the internal function first
  IF to_regprocedure('public._claim_login_streak_reward_internal(uuid)') IS NOT NULL THEN
    DROP FUNCTION public._claim_login_streak_reward_internal(uuid);
  END IF;
  
  IF to_regprocedure('public.claim_login_streak_reward(uuid)') IS NOT NULL THEN
    DROP FUNCTION public.claim_login_streak_reward(uuid);
  END IF;
END
$$;
  ...
  IF v_reward_type = 'BOOSTER' THEN
    IF p_target_series_id IS NOT NULL THEN
      ...
      WHERE b.series_id = p_target_series_id
        ...
        AND (
          b.is_daily_only = false
          OR b.is_daily_only = true
        )
      LIMIT 1;
...
AS $$
BEGIN
  RETURN public._claim_login_streak_reward_internal(
    p_user_id,
    p_target_series_id
  );
END;
