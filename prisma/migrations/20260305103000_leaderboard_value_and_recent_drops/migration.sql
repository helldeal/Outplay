-- Leaderboard: score based on owned card values, and recent drops feed.
...
CREATE OR REPLACE FUNCTION public.get_recent_drops(p_limit int DEFAULT 12)
RETURNS TABLE (
  opening_id uuid,
  user_id uuid,
  username text,
...
REVOKE ALL ON FUNCTION public.get_recent_drops(int) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_recent_drops(int) TO authenticated;
