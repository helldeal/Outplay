-- Adjust card completer pricing:
-- - Use average premium probability of still-missing rarities
-- - Keep a one-card logical base (premium booster price / 5) then apply +10%
...
DECLARE
  ...
  v_per_card_base_price numeric := 0;
  ...
  IF v_missing_count > 0 THEN
    ...
    IF v_rarity_count_for_avg > 0 THEN
      v_average_probability := (
        (CASE WHEN v_missing_legends > 0 THEN v_prob_legends ELSE 0 END) +
        (CASE WHEN v_missing_world_class > 0 THEN v_prob_world_class ELSE 0 END) +
        (CASE WHEN v_missing_champion > 0 THEN v_prob_champion ELSE 0 END) +
        (CASE WHEN v_missing_challenger > 0 THEN v_prob_challenger ELSE 0 END) +
        (CASE WHEN v_missing_rookie > 0 THEN v_prob_rookie ELSE 0 END)
      ) / v_rarity_count_for_avg::numeric;
    ELSE
      v_average_probability := 0;
    END IF;

    v_per_card_base_price := v_premium_booster.price_pc::numeric / 5.0;

    v_dynamic_price :=
      v_per_card_base_price
