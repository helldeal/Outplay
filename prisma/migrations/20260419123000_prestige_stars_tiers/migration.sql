-- Prestige tiers update:
-- 2x copies  => 1 star => x1.25 score multiplier
-- 5x copies  => 2 stars => x1.75 score multiplier
-- 10x copies => 3 stars => x3.00 score multiplier
  ...
  SELECT
    ...
    (
      ROUND(
        COALESCE(
          SUM(
            cwv.weighted_pc_value::numeric *
            CASE
              WHEN GREATEST(COALESCE(ucc.copies_count, 0), 1) >= 10 THEN 3.00::numeric
              WHEN GREATEST(COALESCE(ucc.copies_count, 0), 1) >= 5 THEN 1.75::numeric
              WHEN GREATEST(COALESCE(ucc.copies_count, 0), 1) >= 2 THEN 1.25::numeric
              ELSE 1.00::numeric
            END
          ),
          0::numeric
...
AS $$
  ...
  SELECT
    ...
    CASE
      WHEN GREATEST(COALESCE(ucc.copies_count, 0), 1) >= 10 THEN 3
      WHEN GREATEST(COALESCE(ucc.copies_count, 0), 1) >= 5 THEN 2
      WHEN GREATEST(COALESCE(ucc.copies_count, 0), 1) >= 2 THEN 1
      ELSE 0
    END AS prestige_stars,
    (GREATEST(COALESCE(ucc.copies_count, 0), 1) >= 2) AS is_prestige,
