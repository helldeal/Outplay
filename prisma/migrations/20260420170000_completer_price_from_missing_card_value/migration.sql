-- Card completer pricing refinement:
-- - Keep adjusted probabilities on remaining rarities
-- - Base price on expected value of remaining cards (not premium booster price)
-- - Keep +50% margin logic
...
DECLARE
  ...
  v_avg_value_rookie numeric := 0;
  v_avg_value_challenger numeric := 0;
  v_avg_value_champion numeric := 0;
  v_avg_value_world_class numeric := 0;
  v_avg_value_legends numeric := 0;
  v_missing_avg_card_value numeric := 0;
...
BEGIN
  ...
  WITH missing AS (
    SELECT c.id, c.rarity, c.pc_value
    FROM public.cards c
  ...
  SELECT
    ...
    AVG(pc_value) FILTER (WHERE rarity = 'ROOKIE'::public."Rarity"),
    AVG(pc_value) FILTER (WHERE rarity = 'CHALLENGER'::public."Rarity"),
    AVG(pc_value) FILTER (WHERE rarity = 'CHAMPION'::public."Rarity"),
    AVG(pc_value) FILTER (WHERE rarity = 'WORLD_CLASS'::public."Rarity"),
    AVG(pc_value) FILTER (WHERE rarity = 'LEGENDS'::public."Rarity"),
  ...
  INTO
    ...
    v_missing_legends,
    v_avg_value_rookie,
    v_avg_value_challenger,
    v_avg_value_champion,
    v_avg_value_world_class,
    v_avg_value_legends,
    v_missing_card_ids
  FROM missing;
  ...
  IF v_missing_count > 0 THEN
    ...
    v_missing_avg_card_value :=
      COALESCE(v_adj_prob_legends * COALESCE(v_avg_value_legends, 0), 0) +
      COALESCE(v_adj_prob_world_class * COALESCE(v_avg_value_world_class, 0), 0) +
      COALESCE(v_adj_prob_champion * COALESCE(v_avg_value_champion, 0), 0) +
      COALESCE(v_adj_prob_challenger * COALESCE(v_avg_value_challenger, 0), 0) +
      COALESCE(v_adj_prob_rookie * COALESCE(v_avg_value_rookie, 0), 0);

    IF v_missing_avg_card_value <= 0 THEN
      v_missing_avg_card_value := v_premium_booster.price_pc;
    END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'missingAverageCardValue', ROUND(v_missing_avg_card_value, 4)
  );
END;
$$;
